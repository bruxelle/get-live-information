...
Local tests use mock X JSON files only. X API credit purchase is currently unavailable, so real X API calls should stay disabled. For intentional Notion sync testing, keep using `mock_posts` as input and set `DRY_RUN=false` only for that command.
...
X API credit purchase is currently unavailable, so development should continue with:
